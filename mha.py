...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        # (N, num_heads, L, L)
        qk = torch.matmul(q, k.transpose(-2, -1) / math.sqrt(self.kdim // self.num_heads))
        if mask is not None:
...
if __name__ == '__main__':
    ...
    ma = MultiHeadAttention(embed_dim=embed_dim).to(device)
    x = torch.rand(batch_size, seq_len, embed_dim).to(device)
    y = ma(q=x, k=x, v=x)
    print(f"Input shape: {x.shape}")
    print(f"Output shape: {y[0].shape}, Weights shape: {y[1].shape}")
    print("\nDone.")
